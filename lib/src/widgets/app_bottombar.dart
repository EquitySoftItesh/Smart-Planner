...
class BottomBarInActiveItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox.fromSize(
      size: const Size(kCircleRadius * 2.4, kCircleRadius * 2),
...
class BottomBarPainter extends CustomPainter {
  ...
  void _drawBar(Canvas canvas, Size size) {
    ...
    final path = Path()
      ...
      ..relativeArcToPoint(
        const Offset((kCircleRadius + kCircleMargin) * 1.8, 0.0),
  ...
  void _drawFloatingCircle(Canvas canvas) {
    final path = Path()
      ..addArc(
        Rect.fromCircle(
          center: Offset(
            position + kCircleMargin + kCircleRadius - 4,
            kMargin + kCircleMargin / 10,
          ),
